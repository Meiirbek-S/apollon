...
function statusRu(status: string): string {
  if (status === 'detected') return 'Обнаружено'
  if (status === 'undetected') return 'Не обнаружено'
  if (status === 'unable_to_process') return 'Не удалось обработать'
  return status
}
...
export function FileReportView({ report }: { report: FileReport }) {
  ...
  return (
    <section className="card file-report">
      ...
      <div className="card soft">
        <h3>Антивирусные результаты</h3>
        {report.antivirus_detection && report.antivirus_detection.length > 0 ? (
          <div className="plain-list">
            {report.antivirus_detection.map((item) => (
              <p key={`${item.engine}-${item.status}`}>
                <b>{item.engine}:</b> {statusRu(item.status)}
                {item.threat_name ? ` — ${item.threat_name}` : ''}
              </p>
            ))}
          </div>
        ) : (
          <p className="muted">Нет данных антивирусных движков.</p>
        )}
      </div>
