...
def upload_file_submission(file: UploadFile = File(...), db: Session = Depends(get_db)) -> SubmissionCreateResponse:
    ...
    try:
        ...
        message = "database schema is outdated, run: alembic upgrade head"
        if "yara_" in str(exc).lower():
            message = "database schema is outdated for YARA fields, run: alembic upgrade head"
