# Шаг 7: Подготовка Dynamic Analysis (VirtualBox) без запуска malware
...
- `submission.process_dynamic` (пока без VirtualBox вызова),
...
4. Подготавливаем интерфейс VirtualBox controller (adapter layer):
- `prepare_vm()`
- `restore_snapshot()`
- `copy_sample_in()`
- `execute_in_guest()`
...
- не подключаем реальный VBoxManage,
...
- в коде есть безопасный adapter-контур под VirtualBox,
...
Подключение реального VirtualBox controller:
- snapshot restore,
- guest execution,
- artifact collection,
